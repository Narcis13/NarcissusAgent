...
must_haves:
  truths:
    - "User can import SupervisorContext type for type-safe supervisor implementations"
    - "System can spawn claude -p and receive structured output within timeout"
    - "System can parse [COMPLETE], [ABORT], [CONTINUE] markers from supervisor text"
  artifacts:
...
Note: SupervisorContext intentionally excludes iterationCount/maxIterations as those are managed in the createClaudeSupervisor closure for iteration tracking.
  </action>
  <verify>bun build src/supervisor/types.ts --target=bun</verify>
  <done>types.ts exports SupervisorContext (without iteration fields), SpawnResult, ParsedResponse, ClaudeSupervisorConfig</done>
</task>
...
<success_criteria>
1. types.ts exports all four types (SupervisorContext without iteration fields, SpawnResult, ParsedResponse, ClaudeSupervisorConfig with maxConsecutiveFailures)
