...
must_haves:
  truths:
    - "User sees 'Iteration N/M' in supervisor logs showing budget tracking"
    - "System stops autonomously when iteration budget exhausted"
    - "System recovers from supervisor spawn failures and aborts after 3 consecutive failures"
  artifacts:
...
- iterationCount and maxIterations as direct parameters (not from context)
...
- Clear marker instructions with examples
  ...
  <done>prompt.ts exports buildSupervisorPrompt that accepts iterationCount and maxIterations as direct parameters</done>
</task>
...
- Tracks consecutive failures in closure - aborts after maxConsecutiveFailures (default 3)
- Resets failure counter on successful spawn
...
- /clear command injected on abort for clean state
  ...
  <done>claude.ts exports createClaudeSupervisor factory that enforces iteration budget and aborts after consecutive failures</done>
</task>
...
<success_criteria>
1. prompt.ts exports buildSupervisorPrompt with iteration "N/M" format, accepting iterationCount and maxIterations as direct parameters
2. claude.ts exports createClaudeSupervisor that:
   - Tracks iterations in closure
   - Tracks consecutive failures in closure
   - Aborts after maxConsecutiveFailures (default 3)
