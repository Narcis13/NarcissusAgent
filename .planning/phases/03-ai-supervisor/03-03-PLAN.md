...
must_haves:
  truths:
    - "CLI uses createClaudeSupervisor instead of mock by default"
    - "User can observe supervisor reasoning in logs (via debug mode)"
    - "System stops after hitting iteration budget"
  artifacts:
    - path: "src/index.ts"
...
5. Add logging for supervisor decisions. The existing onSupervisorDecision handler already logs via debugLog - verify it includes confidence and reasoning.
...
2. Help shows new flags:
   ```bash
   bun run src/index.ts --help
   ```
   Should show --mock-supervisor and --max-iterations options.

3. Import chain check:
   ```bash
   bun -e "
   import { createClaudeSupervisor } from './src/supervisor';
   import { createClaudeSupervisor as fromTypes } from './src/types';
   console.log('Direct import:', typeof createClaudeSupervisor);
   console.log('Types import:', typeof fromTypes);
   "
   ```

4. Dry-run startup (no Claude spawn):
   ```bash
   timeout 2 bun run src/index.ts --help || true
   ```

Note: Full integration test requires Claude CLI to be available and would spawn actual processes. The verification ensures the code compiles and imports correctly.
  </action>
  <verify>bun build src/index.ts --target=bun</verify>
  <done>All builds pass, help shows new flags, import chain verified</done>
</task>
